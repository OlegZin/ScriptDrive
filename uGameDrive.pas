...
/// работа с объектами этажей
        ...
        procedure ProcessFloor;      /// обработка процесса мыслей для вызова в CheckStatus, списывает автодействия или пул
        function PlayerMakeFloor: boolean;   /// непосредственное проведение атаки со всеми событиями
...
begin
    ...
    /// если речь о числе больше 10 тысяч
...
begin
    ...
    begin
        ...
        GetEffect('buffSPEED'); // читаем текущее значение, чтобы автоматом списать единичку с оставшегося количества
...
begin
    ChangePool('floor', 1);                // ковыряние объекта в пул
    GameData.S['state.CurrFloorObject'] := id;
                                           // запоминаем этаж и id ковыряемого объекта
    SetModeToUpdate( INT_FLOOR );
end;

function TGameDrive.PlayerMakeFloor: boolean;   /// непосредственное проведение атаки со всеми событиями
var
...
begin
    ...
    begin
        ...
        begin
            ...
            scriptName := obj.S['effects['+IntToStr(index)+']'];    // имя случайного эффекта
            Script.Exec( GameData.S['floorScripts.'+scriptName] );  // выполнение скрипта эффекта
        end;

        /// удаляем объект с этажа
...
begin
    if GetPool('Floor') > 0 then  // если есть действия авто ли пула раздумий
