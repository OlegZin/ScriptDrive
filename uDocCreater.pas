...
function TDocCreater.SaveToPDFFile(_FileName: string): boolean;
var
    w: OleVariant;
    f: string;
    ff: pwidechar;
...
{    SaveToFile(_FileName);
    ...
    try
        ...
        if Vartype(w)=VarDispatch then
        begin
            if FileExists(f) then DeleteFile(ff);

            w.Documents.Add(_FileName);
            w.ActiveDocument.ExportAsFixedFormat(f,wdExportFormatPDF,true,
