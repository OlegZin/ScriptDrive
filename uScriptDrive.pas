...
begin
    ...
    line := CalcMath(line); /// текущая строка может быть математическим выражением без функций
...
//    mth := Trim( copy(command, beg, pos(']', command)-beg) );
    ...
    mth := '('+mth+')'; /// заворачиваем в скобки, чтобы на последней итерации посчитать все выражение целиком (регулярка нашла его)
